...
├── assets/vendor/three/    # Three.js r160 مستضافة محلياً (لا CDN) — انظر tools/
├── tools/                  # سكربتات صيانة: جلب/تحقق Three.js، وفحص معرّفات الـ DOM
...
| 📳 اهتزاز | الإعدادات |
| ⌨️ إعادة تعيين المفاتيح | الإعدادات — اضغط زر المفتاح ثم اضغط المفتاح الجديد، مع زر لاستعادة الافتراضي |
| ⌨️ تصفّح بالكيبورد | كل بطاقات الاختيار (الوضع/المركبة/المرحلة) لها تركيز وتُفعَّل بـ Enter أو مسافة |

## 🔒 الأمان والخصوصية

- **لا تُجمع أي بيانات**: لا تحليلات ولا تتبّع ولا كوكيز. كل التقدّم في `localStorage` على جهازك.
- **سلسلة التوريد**: Three.js **مستضافة محلياً بالكامل** في `assets/vendor/three` (النواة + كل الوحدات
  التي تستوردها اللعبة)، فلا يوجد أي أصل خارجي تثق به الصفحة، و**CSP** يقصر كل شيء على هذا الأصل وحده.
  كانت الوحدات تُحمَّل من `cdn.jsdelivr.net` بهاشات **SRI** في `importmap` — لكن حقل `integrity` داخل
  الـ importmap لا تدعمه إلا متصفحات Chromium، فكانت Safari وFirefox تجلبها **بلا أي تحقق تكامل**.
  البايتات المستضافة مطابقة لهاشات الإصدار الرسمية، ويعيد `tools/verify-vendor.mjs` التحقق منها في CI.
...
- [x] 📅 التحدي اليومي صار **واحداً فعلاً للجميع**: التوليد مفهرس بالمسافة لا بالإطارات، وأحجام
      العقبات (وصناديق تصادمها) والعواصف كلها مبذورة، والمركبة موحّدة
- [x] 🛡️ نافذة الحصانة تمرّ من العقبة بدل أن تمحوها (كانت تسع أمتار من التدمير المجاني)
- [x] ⌨️ إعادة تعيين المفاتيح، وقوائم الاختيار صارت قابلة للتشغيل بالكيبورد بالكامل
- [x] ⏭ لقطة الهزيمة صارت أقصر وقابلة للتخطي بأي ضغطة
